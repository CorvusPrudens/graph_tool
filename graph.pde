...
JSONObject lines2json_step(Line lines[], int lineSteps, PVector axes, PVector startAxes, String[] names) {
  JSONObject out = new JSONObject();
  ...
  for (int i = 0; i < lines.length; i++) {
    ...
    for (int j = 0; j < interp.length; j++) {
      // Don't actually need x pos:
      // line.append(interp[j].x);
      line.append(interp[j].y);
    }

    out.setJSONArray(names[i], line);
  }
...
class Graph {
  ...
  Graph(float x, float y, int _size, int _ticks) {
    ...
    this.numTicks = _ticks;
  }

  void ticks() {
    float step = (float) size / this.numTicks;
    strokeWeight(1);
    stroke(dark);
    fill(dark);
    for (int i = 1; i <  this.numTicks; i++) {
      ...
      rotate(-PI/4);
      String tm = nf((1.0 /  this.numTicks) * i, 0, 2);
      text(tm, 0, 0);
      popMatrix();

      tm = nf(1 - (1.0 /  this.numTicks) * i, 0, 2);
  ...
  void mousePos(int press) {
    ...
    if (mouseX < this.right + 40 && mouseX > this.left - 40 && mouseY > this.up - 40 && mouseY < this.down + 40) {
      ...
      for (int i = 0; i <  this.numTicks + 1; i++) {
  ...
  void save(String file1, String file2) {
    ...
    output = lines2json_step(this.lines, 100, this.axes, this.axesStart, this.names);
